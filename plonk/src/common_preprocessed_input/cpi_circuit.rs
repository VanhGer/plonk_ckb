...
impl CPICircuit {
    ...
    pub fn add_addition_gate(mut self, a: (usize, usize), b: (usize, usize), c: (usize, usize), pi: Fr) -> Self {
        self.gates.push(Gate::new_add_gate(Position::Pos(a.0, a.1), Position::Pos(b.0, b.1), Position::Pos(c.0, c.1), Some(pi)));
        self
    }

    /// Adds a multiplication gate to the circuit.
    pub fn add_multiplication_gate(mut self, a: (usize, usize), b: (usize, usize), c: (usize, usize), pi: Fr) -> Self {
        self.gates.push(Gate::new_mul_gate(Position::Pos(a.0, a.1), Position::Pos(b.0, b.1), Position::Pos(c.0, c.1), Some(pi)));
        self
    }

    /// Adds a constant gate to the circuit.
    pub fn add_constant_gate(mut self, a: (usize, usize), b: (usize, usize), c: (usize, usize), value: Fr, pi: Fr) -> Self {
        self.gates.push(Gate::new_constant_gate(Position::Pos(a.0, a.1), Position::Pos(b.0, b.1), Position::Pos(c.0, c.1), value, Some(pi)));
        self
    }
