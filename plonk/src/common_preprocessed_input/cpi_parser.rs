...
impl CPIParser {
    ...
    fn prepare_generation(
        ...
        let result = string.split('=').map(|s| s.to_string()).collect::<Vec<String>>();
    ...
    fn gen_circuit(
        ...
        let mut position_map = position_map.into_iter()
            .map(|(key, mut vec)| {
                vec.reverse();
                vec.rotate_right(1);
                (key, vec)
        ...
        for gate in gate_list.iter() {
            let left = position_map.get_mut(&gate.left.value_string)
                .unwrap().pop().unwrap();
            let left = (left.0, left.1);
            let right = position_map.get_mut(&gate.right.value_string)
                .unwrap().pop().unwrap();
            let right = (right.0, right.1);
            let bottom = position_map.get_mut(&gate.bottom.value_string)
                .unwrap().pop().unwrap();
    ...
    fn generate_additional_gate(
        ...
        let gate = ParserGate::new(left.clone(), right.clone(), result.clone(), TypeOfCircuit::Addition);
    ...
    fn generate_constant_gate(
        ...
        let gate = ParserGate::new(value.clone(), right.clone(), result.clone(), TypeOfCircuit::Constant);
